...
class Batch():
    def __init__(self, dataset, source, target, device, vocabulary, train=True):
    ...
    def __next__(self):
        ...
        for batch in self.dataset:
            ...
            if self.vocabulary is not None:
                src_mask = self.create_src_mask(source_batch, self.vocabulary)
                tgt_mask = self.create_tgt_mask(target_batch, self.vocabulary) 
    ...
    """
    @staticmethod
    def create_src_mask(batch, vocabulary):
        """
        ...
        pad_index = vocabulary.stoi['<pad>']
        return (batch != pad_index).unsqueeze(1).unsqueeze(1)
    
    @staticmethod
    def create_tgt_mask(batch, vocabulary):
        """
        ...
        mask2 = (torch.from_numpy(mask2).to(batch.get_device())) == 0
